...
It is required to leverage tunnel-operator security tools capabilities by adding the support for building compliance reports
...
Upon tunnel-operator start cluster compliance reconcile loop will track the deployed spec file ,nsa-1.0 spec and evaluation the cron expression in spec file,
if  the cron interval matches , tunnel-operator will generate the compliance and compliance detail reports :
...
- tunnel-operator will iterate all fetched raw data and find a match by `ID`
...
it is required to update `02-tunnel-operator.rbac.yaml` rules to include new permissions
...
| Scan container images vulnerabilities                                                         | scan container for vulnerabilities<br/> and misconfiguration                                            | Pod,ReplicationController,ReplicaSet,<br/>StatefulSet,DaemonSet,Job,CronJob | Tunnel       | Tunnel                                                                                                                         |
