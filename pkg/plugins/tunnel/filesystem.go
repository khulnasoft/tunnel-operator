...
func GetPodSpecForStandaloneFSMode(ctx tunneloperator.PluginContext, config Config, workload client.Object, _ map[string]docker.Auth, securityContext *corev1.SecurityContext, p *plugin, clusterSboms map[string]v1alpha1.SbomReportData) (corev1.PodSpec, []*corev1.Secret, error) {
	...
	initContainerDB := corev1.Container{
		...
		ImagePullPolicy:          corev1.PullPolicy(config.GetImagePullPolicy()),
		TerminationMessagePolicy: corev1.TerminationMessageFallbackToLogsOnError,
		Env:                      initContainerFSEnvVar(tunnelConfigName, config),
		Command: []string{
			"tunnel",
		},
		Args: []string{
	...
	for _, c := range getContainers(spec) {
		...
		fscommand := []string{SharedVolumeLocationOfTunnel}
...
func GetPodSpecForClientServerFSMode(ctx tunneloperator.PluginContext, config Config, workload client.Object, _ map[string]docker.Auth, securityContext *corev1.SecurityContext, p *plugin, clusterSboms map[string]v1alpha1.SbomReportData) (corev1.PodSpec, []*corev1.Secret, error) {
	...
	for _, c := range getContainers(spec) {
		...
		fscommand := []string{SharedVolumeLocationOfTunnel}
		args := getFSScanningArgs(ctx, command, ClientServer, encodedTunnelServerURL.String())
		if len(clusterSboms) > 0 { // tunnel sbom ...
			if sbomreportData, ok := clusterSboms[c.Name]; ok {
				secretName := fmt.Sprintf("sbom-%s", c.Name)
				secret, err := CreateSbomDataAsSecret(sbomreportData.Bom, secretName)
				...
				fileName := fmt.Sprintf("%s.json", secretName)
				CreateVolumeSbomFiles(&volumeMounts, &volumes, &secretName, fileName)
				fscommand, args = GetSbomFSScanningArgs(ctx, ClientServer, encodedTunnelServerURL.String(), fmt.Sprintf("/sbom/%s", fileName))
			}
		}
		containers = append(containers, corev1.Container{
			Name:                     c.Name,
...
func getFSScanningArgs(ctx tunneloperator.PluginContext, command Command, mode Mode, tunnelServerURL string) []string {
	c, err := getConfig(ctx)
	if err != nil {
		return []string{}
	}
...
func GetSbomFSScanningArgs(ctx tunneloperator.PluginContext, mode Mode, tunnelServerURL string, sbomFile string) ([]string, []string) {
	command := []string{
		SharedVolumeLocationOfTunnel,
	}
	c, err := getConfig(ctx)
...
func initContainerFSEnvVar(tunnelConfigName string, config Config) []corev1.EnvVar {
	envs := []corev1.EnvVar{
		constructEnvVarSourceFromConfigMap("HTTP_PROXY", tunnelConfigName, keyTunnelHTTPProxy),
		constructEnvVarSourceFromConfigMap("HTTPS_PROXY", tunnelConfigName, keyTunnelHTTPSProxy),
		constructEnvVarSourceFromConfigMap("NO_PROXY", tunnelConfigName, keyTunnelNoProxy),
