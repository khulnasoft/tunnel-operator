...
func TestPlugin_GetScanJobSpec(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			config: map[string]string{
				"trivy.repository":                "docker.io/khulnasoft/trivy",
				"trivy.tag":                       "0.35.0",
				"trivy.mode":                      string(trivy.Standalone),
				"trivy.dbRepository":              trivy.DefaultDBRepository,
				"trivy.javaDbRepository":          trivy.DefaultJavaDBRepository,
				...
				InitContainers: []corev1.Container{
					{
						...
						Image:                    "docker.io/khulnasoft/trivy:0.35.0",
...
func TestPlugin_ParseReportData(t *testing.T) {
	config := &corev1.ConfigMap{
		...
		Data: map[string]string{
			"trivy.repository": "khulnasoft/trivy",
			"trivy.tag":        "0.9.1",
		},
	}
	...
	testCases := []struct {
	...
	}{
		{
			...
			configData: map[string]string{
				"trivy.dbRepository":     trivy.DefaultDBRepository,
				"trivy.javaDbRepository": trivy.DefaultJavaDBRepository,
				"trivy.repository":       "gcr.io/khulnasoft/trivy",
				"trivy.tag":              "0.35.0",
				"trivy.mode":             string(trivy.Standalone),
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			pluginContext := tunneloperator.NewPluginContext().
				...
				WithServiceAccountName("tunneloperator-sa").
				WithClient(fakeclient).
				WithTrivyOperatorConfig(map[string]string{tunneloperator.KeyVulnerabilityScansInSameNamespace: "true"}).
				Get()
			resolver := kube.NewObjectResolver(fakeclient, &kube.CompatibleObjectMapper{})
			instance := trivy.NewPlugin(fixedClock, ext.NewSimpleIDGenerator(), &resolver)
