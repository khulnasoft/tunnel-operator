...
func (p *plugin) getCommandAndArgs(ctx tunneloperator.PluginContext, mode Mode, imageRef string, trivyServerURL string, resultFileName string) ([]string, []string) {
	command := []string{
		"trivy",
	}
...
func (p *plugin) getFSScanningArgs(ctx tunneloperator.PluginContext, command Command, mode Mode, trivyServerURL string) []string {
	c, err := p.getConfig(ctx)
	if err != nil {
		return []string{}
	}
